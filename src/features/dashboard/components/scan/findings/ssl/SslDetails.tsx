...
export function SslDetails({
  backHref = '/scan/report/findings/ssl',
  scanId,
  findingId,
}: SslDetailsProps) {
  ...
  const renderTechnicalDetails = (detail: unknown) => {
    ...
    const d = detail as SslTechnicalDetail;
    ...
    return (
      <div className="space-y-2 font-mono text-xs text-[#4B5563]">
        ...
        {d.validFrom && (
          <div>
            <span className="font-semibold">Valid From:</span> {d.validFrom}
          </div>
        )}
        {d.validTo && (
          <div>
            <span className="font-semibold">Valid To:</span> {d.validTo}
          </div>
        )}
        {d.issuer && (
          <div>
            <span className="font-semibold">Issuer:</span> {d.issuer}
          </div>
        )}
        {d.subject && (
          <div>
