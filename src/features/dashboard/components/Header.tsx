...
export function DashboardHeader() {
  ...
  return (
    <>
      <header className="h-[64px] bg-[#F5F5F5] flex items-center justify-between px-4 md:px-6 shrink-0 z-30">
        {/* Mobile Header: Logo (left) + Hamburger (right) */}
        <div className="flex lg:hidden items-center justify-between w-full">
          <Link href="/dashboard">
            <Image
              src="/images/logo-sidebar-mobile.png"
              alt="VulnWatch AI"
              width={140}
              height={32}
        ...
        {/* Search bar */}
        <div className="hidden md:flex items-center gap-2 bg-white border border-[#E5E7EB] rounded-lg px-3 py-2 w-[280px]">
      ...
      />
      <div
        className="fixed inset-y-0 left-0 z-50 w-[260px] bg-white shadow-xl flex flex-col lg:hidden transition-transform duration-300"
        style={{ transform: mobileMenuOpen ? "translateX(0)" : "translateX(-100%)" }}
      >
        <div className="flex items-center justify-between h-[64px] px-4 border-b border-[#E5E7EB]">
          <span className="font-semibold text-primary text-lg">VulnWatch AI</span>
          <button
            type="button"
            onClick={() => setMobileMenuOpen(false)}
